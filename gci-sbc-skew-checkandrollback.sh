...
if grep -Fxq "$SEARCH_LINE" "$FILE"; then
   ...
   get_server_info   
   echo "Skew parameter value is set, action will now be taken to remove" | tee -a $LOG_FILE
   skew_removal

   echo "Rollback completed,Skew parameter has been removed" | tee -a $LOG_FILE
