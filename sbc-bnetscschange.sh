...
LOG_FILE=/tmp/SBC_BNETSCS-CHANGE-$HOST_NAME.log
...
if [ -f $FILE ]; then
    echo "bnetscs file found "
	echo | tee -a "$LOG_FILE"
else
    echo "bnetscs file information not found in directory $DIR "
	 echo "Please contact support to get file to add to the folder"
	echo | tee -a "$LOG_FILE"
    exit 1
fi
...
if [ -f $FILE2 ]; then
    echo "bnetscs.original file already exists in /opt/bnet/bin script will not proceed and will end"
	echo | tee -a "$LOG_FILE"
	exit 1
else
    echo "bnetscs.original not found, script will proceed"	 
	echo | tee -a "$LOG_FILE"
...
echo "renaming bnetscs file" | tee -a $LOG_FILE

mv bnetscs bnetscs.original

echo | tee -a "$LOG_FILE"

echo "coping bnetscs file" | tee -a $LOG_FILE

cp $FILE /opt/bnet/bin
...
mv $LOG_FILE /tmp/SBC_BNETSCS-CHANGE-$HOST_NAME-$theIPaddress-$theSerial-$(date +"%Y_%m_%d_%I_%M_%p").log
